...
app.get('/spells/:id', (req, res) => {
   ...
   const { id } = req.params;
   const spells = ApiData.spells.find((spell) => spell.id === parseInt(id))
    console.log(spells)
       res.json(spells)             
});
...
app.get('/characters', (req, res) => {
    ...
    const result = ApiData.characters.filter((characters)=>{
        const boolhogwartsStudent = hogwartsStudent.toLocaleLowerCase() === 'true';
        return characters.hogwartsStudent == boolhogwartsStudent ||
            characters.hogwartsHouse == hogwartsHouse;
